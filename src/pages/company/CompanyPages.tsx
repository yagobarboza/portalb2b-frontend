...
import { mockCustomers, mockOrders, mockProducts, customerName } from '../../data/mock';
...
export function CatalogPage() {
  ...
  const filtered = products.filter(
    (p) => p.name.toLowerCase().includes(search.toLowerCase()) || (p.category_id ?? '').toLowerCase().includes(search.toLowerCase())
  );
  const createProduct = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const novo: Product = {
      id: `prod-${Date.now()}`,
      sku: `TM-${Date.now().toString().slice(-6)}`,
      code: null,
      ...
      description: String(form.get('description') || 'Novo produto cadastrado'),
      status: 'active',
      ...
      prev.map((p) =>
        p.id === editProduct.id
          ? {
              ...p,
              name: String(form.get('name')),
              category_id: String(form.get('category')) || p.category_id,
              price: Number(form.get('price')),
              description: String(form.get('description') || (p.description ?? '')),
              stock: Number(form.get('stock')) || (p.stock ?? 0),
              image_url: preview || p.image_url,
            }
          : p
      )
    );
              ...
              <form onSubmit={createProduct} className="space-y-4">
                ...
                <div className="rounded-lg border border-dashed p-4 text-center">
                  ...
                  <Button type="button" variant="outline" onClick={() => fileRef.current?.click()}>
                    <FileUp className="mr-2 h-4 w-4" />Escolher foto local
                  </Button>
                </div>
          ...
          <TableBody>
            {filtered.map((p) => (
              <TableRow key={p.id}>
                ...
                <TableCell><Badge variant="secondary">{p.category_id ?? '—'}</Badge></TableCell>
                <TableCell className="font-mono text-xs text-muted-foreground">{p.sku}</TableCell>
                <TableCell className="font-semibold">{formatCurrency(p.price)}</TableCell>
                <TableCell>{p.stock ?? 0} un.</TableCell>
        ...
        <DialogContent className="max-w-lg">
          ...
          {editProduct && (
            <form onSubmit={updateProduct} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2"><Label>Categoria</Label><Input name="category" defaultValue={editProduct.category_id ?? ''} required /></div>
                <div className="space-y-2"><Label>Estoque</Label><Input name="stock" type="number" defaultValue={editProduct.stock ?? 0} required /></div>
              </div>
              ...
              <div className="rounded-lg border border-dashed p-4 text-center">
                ...
                {preview || editProduct.image_url ? (
                  <img src={preview || (editProduct.image_url ?? undefined)} alt="Prévia" className="mx-auto h-24 rounded object-cover" />
                ) : (
                  <Upload className="mx-auto mb-2 h-6 w-6 text-muted-foreground" />
                )}
