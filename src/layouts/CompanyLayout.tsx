...
const menuItems = [
  ...
  // ✅ Pagamentos & Assinatura (billing Asaas) — exige apenas usuário da
...
export default function CompanyLayout() {
  ...
  // Logo GRANDE (h-14 w-14) e SEM o texto do nome da empresa.
  const brand = (
    <div className="flex items-center">
      {logoUrl ? (
        <img src={logoUrl} alt={branding?.name ?? 'nydB2B'} className="h-14 w-14 rounded-xl object-contain" referrerPolicy="no-referrer" />
      ) : (
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r bg-background md:flex">
        <div className="flex h-20 items-center border-b px-4">{brand}</div>
      ...
      <div className="md:pl-60">
        ...
        <header className="sticky top-0 z-30 flex h-20 items-center gap-4 border-b bg-background/95 px-4 backdrop-blur">
          <Button
            variant="ghost"
            size="icon"
            className="md:hidden"
          ...
          <div className="flex items-center md:hidden">{brand}</div>
